...
arma::mat runge_kutta(const std::function<arma::vec(double, const arma::vec&)>& f, const arma::vec& y0, double h, double T, const std::string& filename);

// Runge-Kutta method for scalar ODEs
arma::mat runge_kutta(const std::function<double(double, double)>& f, double y0, double h, double T, const std::string& filename);




// Runge-Kutta method for vector ODEs
arma::mat runge_kutta(const std::function<arma::vec(double, const arma::vec&)>& f, const arma::vec& y0, double h, double T, const std::string& filename = "runge_kutta_vector_result.csv") {
...
// Runge-Kutta method for scalar ODEs
arma::mat runge_kutta(const std::function<double(double, double)>& f, double y0, double h, double T, const std::string& filename = "runge_kutta_scalar_result.csv") {
    // Create the vector t containing the jumps
    arma::uword steps = static_cast<arma::uword>(T / h) + 1;
    arma::vec t = arma::linspace(0, T, steps);

    // Initialize the matrix with initial conditions
    arma::mat rk_mat(steps,1);
    ...
    // Perform Runge-Kutta method
    for (arma::uword i = 1; i < t.n_elem; ++i) {
        double k1 = h * f(t(i - 1), rk_mat(i - 1,0));
        double k2 = h * f(t(i - 1) + h / 2, rk_mat(i - 1,0) + k1 / 2);
        double k3 = h * f(t(i - 1) + h / 2, rk_mat(i - 1,0) + k2 / 2);
        double k4 = h * f(t(i - 1) + h, rk_mat(i - 1,0) + k3);

        rk_mat(i,0) = rk_mat(i - 1,0) + (k1 + 2 * k2 + 2 * k3 + k4) / 6;
    }
    arma::mat sol = rk_mat;
    // Append the time vector as the first column
    rk_mat.insert_cols(0,t);
