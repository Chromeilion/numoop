import numpy as np
import numpy.typing as npt
from typing import Sequence, TypeAlias, TypeVar, Callable

# The internal types supported for each column
sup_col_types: TypeAlias = (np.int32 | np.int64 | np.float32 | np.float64 |
                            np.uint32 | np.uint64 | np.complex64 |
                            np.complex128 | complex | int | float)

# Supported types of the actual arrays
sup_col_arr_types: TypeAlias = (
        npt.NDArray[np.int32] | npt.NDArray[np.int64] |
        npt.NDArray[np.float32] | npt.NDArray[np.float64] |
        npt.NDArray[np.uint32] | npt.NDArray[np.uint64] |
        npt.NDArray[np.complex64] | npt.NDArray[np.complex128]
)

# Types supported when appending row-wise
sup_row_types: TypeAlias = Sequence[sup_col_types] | sup_col_arr_types
# When specifying multiple rows at once
sup_multi_row_types: TypeAlias = (Sequence[sup_row_types] |
                                  sup_col_arr_types)

# Int to category for categorical cols
cat_dict: TypeAlias = dict[int, str]
# Container for all cat dictionaries
all_cat_dict: TypeAlias = dict[int, cat_dict]

# Supported type of vector
sup_vec = npt.NDArray[np.float64]
# Supported type for vector function
sup_fun_vec = Callable[[float, sup_vec], sup_vec]
# Supported type for vector function
sup_fun_scal = Callable[[float, float], float]
