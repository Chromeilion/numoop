...
public:
    ...
    arma::mat method_vec(arma::mat& result_mat, const arma::vec& y0, 
                         const arma::vec& t, double h) const override {
        ...
        //save_vec(result_mat, t, y0, "result_rk4_vector.csv");
        return result_mat;
    }

    // Implementation for scalar case
    arma::mat method_scalar(arma::mat& result_mat, const arma::vec& t, double h) const override {

        for (arma::uword i = 1; i < t.n_elem; ++i) {
            arma::vec k1 = h * f_scal(t(i - 1), result_mat(0,i - 1));
            arma::vec k2 = h * f_scal(t(i - 1) + h / 2, result_mat(0,i - 1) + k1 / 2);
            arma::vec k3 = h * f_scal(t(i - 1) + h / 2, result_mat(0,i - 1) + k2 / 2);
            arma::vec k4 = h * f_scal(t(i - 1) + h, result_mat(0,i - 1) + k3);
