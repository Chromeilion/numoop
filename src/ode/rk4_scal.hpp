...
public:
    ...
    RK4_Scal(const func_scal f = {}, const T y0 = {}, const T h = {}, const T end = {})
    ...
    arma::Mat<T> method(arma::Mat<T>& result_mat, const arma::Col<T>& t) override {
        ...
        for (arma::uword i = 1; i < t.n_elem; ++i) {
            ...
            T k3 = this->h * this->f(t(i - 1) + this->h / 2, result_mat(0,i - 1) + k2 / 2);
            T k4 = this->h * this->f(t(i - 1) + this->h, result_mat(0,i - 1) + k3);

            result_mat(0,i) = result_mat(0,i - 1) + (k1 + 2 * k2 + 2 * k3 + k4) / 6;
        }
        this->save(result_mat, t, "result_rk4_scalar.csv");
        return result_mat;
    }
};
} // namespace oop::ode_scal
